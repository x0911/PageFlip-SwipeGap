...
-   `swipeGap: number` - default: `5` auxiliary factor. Use it instead of `swipeDistance` to get better results, because changing `swipeDistance` is not effective on all browsers (custom option, not available in the original library)
